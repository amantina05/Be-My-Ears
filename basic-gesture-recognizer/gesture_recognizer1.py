## https://www.freecodecamp.org/news/weekend-projects-sign-language-and-static-gesture-recognition-using-scikit-learn-60813d600e79/
...
## A Machine Learning pipeline that performs hand localization and static-gesture recognition built using the scikit learn and scikit image libraries http://www.ssreehari.com/Sign-language-and-static-gesture-recgnition/
  ...
  # trained the multi classification algorith using Support Vector Classification with a linear kernel
  # to train a modle using sklear yu do
...
## use pandas in the get_data() func to load the CSV file
  # crop() & convertToGrayToHog() are used to get the required hog vector and append it to the list of vectors that we’re building, in order to train the multi-class classifier
...
## this is part two: building the hand/ not dataset
"""
...
##this is to encode the output labels (the Y-values) to numerical values. We do this using sklearn’s label encoder
...
gs = GestureRecognizer('../Dataset')

gs.train(user_tr)
gs.save_model(name = "dataset.pkl.gz", version = "0.0.1", author = 'ss')
print ("The GestureRecognizer is saved to disk")

gs = GestureRecognizer.load_model(name = "dataset.pkl.gz")
# automatic dict unpacking gs.recognize_gesture()
